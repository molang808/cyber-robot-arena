...
최종 유닛은 8분 시점 HP 3,780으로 같은 시점 메가 보스의 약 1.8배다.
...
곱연산 스택에 상한이 없다. 뱀파이어 서바이버즈는 무기 6 + 패시브 6 슬롯 제한으로
성장에 **구조적 상한**을 뒀다. 같은 성격의 제약이 필요하고, 어떤 형태가 맞는지는
캠페인 모드로 다시 측정해서 정한다.
...
**1. 곱연산 스택에 상한이 없다** — 위 [밸런스](#밸런스--측정하고-고친-것) 참고.
최적 선택 플레이어의 DPS가 무작위 선택 대비 81배다. 슬롯 제한이 다음 작업이다.
