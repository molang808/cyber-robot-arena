...
명중률 100%(완벽한 플레이어)를 가정하면 5분 시점 처리량 여유가 **2.47배**까지 올라간다.
즉 잘하는 플레이어에게는 여전히 상한이 없다. 이건 뱀서가 30분 시간 제한과 사신으로 푼 문제고,
이 게임에는 아직 **런 종료 조건이 없다.** 다음 작업이다.
...
**1. 런 종료 조건이 없다** — 위 [밸런스](#밸런스--측정하고-고친-것) 참고. 완벽한 플레이어에게는
아직 상한이 없다. 시간 제한과 최종 보스를 넣는 것이 다음 작업이다.

**1-2. 단위 테스트가 없다** — 밸런스 시뮬레이터가 곡선 전체를 검증하지만,
서브스텝 충돌이나 드랍 테이블 같은 개별 로직에 대한 단위 테스트는 아직 없다.
